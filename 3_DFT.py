...
# Note: The 3-factor section remains unchanged. Update similarly if needed.
    ...
    with st.expander("⚙️ RSM Factor Selection", expanded=True):
        ...
        if len(selected_factors) != 2:
            ...
            design = ccdesign(2, center=(3, 3))  # 2 factors, 3 center points
            design_df = pd.DataFrame(design, columns=selected_factors)

            # After rescaling
            for i, factor in enumerate(selected_factors):
                low, high = factor_ranges[factor]
                design_df[factor] = ((design_df[factor] + 1) / 2) * (high - low) + low
                design_df[factor] = design_df[factor].round(1)  # Round to 1 decimal place

            design_df['energy'] = ""  # Placeholder for results
            design_df.index += 1
