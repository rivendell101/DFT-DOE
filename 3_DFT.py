...
doe_file = st.file_uploader("Upload your DOE CSV with 'energy' column:", type="csv")

if doe_file is not None:
    df_uploaded = pd.read_csv(doe_file)
    st.success("✅ DOE file uploaded.")
    
    # Save both primary and backup files
    df_uploaded.to_csv("Design of Experiments - Sheet6_5_rsm2_v1.csv", index=False)
    df_uploaded.to_csv("rsm_file.csv", index=False)

    with st.expander("🧮 Data Table"):
        st.dataframe(df_uploaded)
else:
    st.info("ℹ️ No file uploaded. Attempting to use backup file.")
    try:
        df_uploaded = pd.read_csv("Design of Experiments - Sheet6_5_rsm2_v1.csv")
        st.success("✅ Loaded backup file.")
        with st.expander("🧮 Data Table"):
            st.dataframe(df_uploaded)
    except FileNotFoundError:
        st.error("❌ No file uploaded and no backup file found.")
        df_uploaded = None

# Proceed if data is available
if df_uploaded is not None:
    st.markdown("### Step 5: RSM ANOVA and Optimization")

    numeric_cols = df_uploaded.select_dtypes(include="number").columns.tolist()
    factor_choices = [col for col in numeric_cols if col != "energy"]

    if len(factor_choices) < 2:
        st.warning("Not enough numeric columns (excluding 'energy') for analysis.")
    else:
        selected_factors = factor_choices[:2]
        st.info(f"Automatically using: **{selected_factors[0]}** and **{selected_factors[1]}**")

        if st.button("Run ANOVA and Optimize"):
            try:
                with st.expander("📊 RSM ANOVA & Model Fit"):
                    # 1. Fit model
                    formula = (
                        f"energy ~ {selected_factors[0]} + {selected_factors[1]} + "
                        f"I({selected_factors[0]}**2) + I({selected_factors[1]}**2) + "
                        f"{selected_factors[0]}:{selected_factors[1]}"
                    )
                    model = ols(formula, data=df_uploaded).fit()

                    # 2. ANOVA Table and Stats
                    anova_table = sm.stats.anova_lm(model, typ=2)
                    ...
                    predicted_energy = result.fun

                    # 5. Show Optimal Results
                    ...
                    st.write(f"**Predicted Energy at Optimum:** {predicted_energy:.4f}")
                    ...
                    fig3d.add_trace(go.Scatter3d(
                        x=[optimal_factors[0]], y=[optimal_factors[1]],
                        z=[predicted_energy],
                        mode='markers+text',
